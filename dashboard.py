            ...
            if show_fred:
                for series_label, series_id in FRED_SERIES.items():
                    if series_id not in FRED_OVERLAY_IDS:
                        continue
                    fred_df = fetch_fred(series_id, (date.today() - timedelta(days=730)).isoformat(), date.today().isoformat())
...
with st.expander("Industry Benchmarks (FRED)"):
    ...
    if selected_fred:
        ...
        for i, label in enumerate(selected_fred):
            ...
            fred_data = fetch_fred(sid, (date.today() - timedelta(days=730)).isoformat(), date.today().isoformat())
            with fred_cols[i % 2]:
                st.markdown(f"**{label}**")
                if fred_data.empty:
